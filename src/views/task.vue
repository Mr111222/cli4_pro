...
<template>
  <div>
    ...
    <div class="btns">
      <p v-for="(item, index) in lists" class="ps" @click="toOne(index)">
        <b>{{ item }}</b>
      </p>
    </div>
    <div class="begin">
        ...
        <el-table-column fixed="right" label="Operations" width="120">
          <template slot-scope="scope">
            ...
            <el-tag
              ...
              size="mini"
              type="success"
              @click="handleClickContinue(scope.$index, scope.row.id)"
              v-else
              >上传</el-tag
            >
            <el-tag
              size="mini"
              type="danger"
              @click="handleClickDel(scope.$index, scope.row.id)"
              >删除</el-tag
            >
          </template>
        </el-table-column>
    ...
    <div>
      ...
      <el-button @click="stopTime">停止⏲</el-button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    createBegin() {
      ...
      console.log(beginListLength, "beginListLength");
